...
class Population {
  constructor(m, num) {
    ...
    for (let i = 0; i < num; i++) {
      let location = createVector(20, height / 2);
      this.population[i] = new Mouse(new DNA(), location);
    }
  }

  start(traps, target) {
    for (let i = 0; i < this.population.length; i++) {
      this.population[i].run(traps, target);
    }
  }
  ...
  reproduction() {
    ...
    for (let i = 0; i < this.population.length; i++) {
      ...
      let location = createVector(20, height / 2);
      this.population[i] = new Mouse(child, location);
    }
    this.generations++;
  }
